...
From today's research-log entries, take candidates with composite score >= 7,
sorted descending by score. For each, in order:
...
- Compute qty: `floor( (target_position_pct / 100) * account_equity / last_price )`.
- Run `./.claude/skills/bull/preflight-check.sh <ticker> buy <qty> <last_price>`.
  If exit non-zero, skip and log the reason.
- Send the buy via `scripts/alpaca.sh`.
...
### Step 5: Commit
`git add -A && git commit -m "market-open YYYY-MM-DD: <n> buys, <m> sells" && git push origin main`

## Non-negotiables

- Never bypass preflight. If preflight fails, the trade does not happen — period.
- Never average down on a losing position.
- Never open a short, an option, or anything in `disallowed_*`.
